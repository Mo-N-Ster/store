...
export function CashierPage({ user, notify }: { user: User; notify: (message: string) => void }) {
  ...
  const loadSide = () =>
    Promise.all([
      storeApi.users().then(setUsers),
      storeApi.invoices({ from: todayIso() }).then(setInvoices),
    ]);
  useEffect(() => {
    void loadSide();
  }, []);
  ...
  const validate = async () => {
    try {
      const result = await storeApi.createInvoice({
        employeeId: user.id,
      ...
      <aside className="pos-sidebar">
        ...
        <div className="history">
          {invoices.map((invoice) => (
            <button key={invoice.id} onClick={() => storeApi.invoice(invoice.id).then(setReceipt)}>
          ...
          {users
            ...
            .map((member) => (
              <button
                ...
                onClick={(event) =>
                  storeApi
                    .attendance({ employeeId: member.id, present: !event.shiftKey })
