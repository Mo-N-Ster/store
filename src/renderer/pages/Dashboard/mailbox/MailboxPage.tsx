...
export function MailboxPage({ user, notify }: { user: User; notify: (x: string) => void }) {
  ...
  const load = () => storeApi.messages({ userId: user.id, role: user.role }).then(setRows);
  useEffect(() => {
    void load();
  }, []);
    ...
    <>
      ...
      {rows.map((message) => (
        <article
          ...
          key={message.id}
          onClick={() => storeApi.markMessage({ id: message.id, isRead: true }).then(load)}
        >
          <b>{message.subject}</b>
          <small>
          ...
          {user.role === 'admin' && (
            <button
              ...
              onClick={(event) => {
                ...
                storeApi.deleteMessage(message.id).then(load);
              }}
            >
              🗑
            </button>
